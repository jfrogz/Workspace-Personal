...
public class RegistroDeClientesYProveedoresController extends MenuController {
    ...
    private void inicializaReportFrame() {
        repParams.clear();
        reportTitle = getProperty("Catalogos.CltsPrvs.ReportTitle");
        reportName = getProperty("Catalogos.CltsPrvs.ReportName");
    ...
    public void inicializaEventos() {
        ...
        flEmpresa.addEventFilter(KeyEvent.KEY_TYPED, Utilerias.permiteLetrasNumeros());
        Utilerias.addTextLimiter(flClave, 100);
        Utilerias.addTextLimiter(flEmpresa, 50);
        ...
        flClave.textProperty().addListener((observable, oldValue, newValue) -> {
            filteredData.setPredicate(clieProv -> {
                ...
                String empresa = flEmpresa.getText().toLowerCase().trim();
                if ((null == clave || clave.isEmpty()) && (null == empresa || empresa.isEmpty())) {
                    return true;
                } else {
                    if (clieProv.getClave().toLowerCase().startsWith(clave) && clieProv.getNombre().toLowerCase().startsWith(empresa)) {
                        return true;
                    }
                }
